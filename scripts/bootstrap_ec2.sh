...
# Installs system packages, X-Ray daemon, CloudWatch agent, and prepares
# the directory structure. The CD workflow handles code deploy + service start.
set -euo pipefail
...
# ── AWS X-Ray daemon ───────────────────────────────────────────────────────────
curl -fsSL https://s3.us-east-2.amazonaws.com/aws-xray-assets.us-east-2/xray-daemon/aws-xray-daemon-3.x.rpm \
  -o /tmp/xray-daemon.rpm
dnf install -y /tmp/xray-daemon.rpm
rm /tmp/xray-daemon.rpm

# X-Ray daemon config — listens on UDP 2000, sends to us-east-1
