...
**Chat-history persistence, write path only** (shipped): `/fpl/ask` accepts an optional `session_id` (the frontend's existing per-thread `ChatSession.id`, `crypto.randomUUID()`) and, when present, upserts a `conversations` row keyed on the new `conversations.client_session_id` column (`db/migrations/003_conversations_client_session_id.sql`) and saves the question/answer pair to `chat_messages` — `server/app_db.upsert_conversation`/`save_chat_messages`. Best-effort: both no-op or are wrapped so a failure only logs `chat_history.persist_failed`, never breaks the chat response. **No read path exists yet** — the sidebar's history is still 100% `localStorage`-driven (`ui/src/storage.ts`), this is write-only groundwork. **Known gap**: `client_session_id` isn't scoped to the requesting `device_token`/`user_id` at write time — low risk today since nothing reads this table back yet and the id is an unguessable UUID, but the eventual read endpoint must add that scoping before trusting this data as access-controlled.

**Still to come**: encrypted BYOK API-key storage gated on sign-in (Phase 3); the actual chat-history read path (list/load past conversations) and retrofitting `client_session_id` ownership checks once that exists.
