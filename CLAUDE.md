...
No user-facing sign-in exists yet. Schema laid down in `db/migrations/002_auth_tables.sql` (also appended to `db/schema.sql`): `users`, `device_tokens`, `conversations`, `chat_messages`. A `conversation` is owned by exactly one of `user_id` or `device_token` (`conversations_owner_check`), so either can be set once a device merges into an account. These 4 tables are deliberately **not** granted to `gaffer_readonly` or `gaffer_etl` — see the `gaffer_app` role gotcha below.

**Anonymous device-token cookie** (shipped): `/fpl/ask` sets a `gaffer_device` cookie (httponly, `secure` in production, `samesite=lax`, 400-day max-age — Chrome's cap) and upserts a `device_tokens` row via `server/device_auth.py`, connected through the new `database_app_url` setting (`DATABASE_APP_URL`, `gaffer_app` role). If `DATABASE_APP_URL` isn't set, `get_or_create_device_token` falls back to an unpersisted token so local dev works without Postgres configured — the cookie still round-trips, it just isn't tracked server-side. Frontend sends `credentials: "include"` on the `/fpl/ask` call (`ui/src/api.ts`) so the cookie persists; no other endpoint touches it yet. **`fpl_team_id` persistence is still `localStorage`-only** (`ui/src/App.tsx`, `ui/src/Landing.tsx`) — the device token doesn't back anything user-visible yet, it's purely infrastructure for the next piece.
