...
No user-facing auth exists yet. Schema laid down in `db/migrations/002_auth_tables.sql` (also appended to `db/schema.sql`): `users`, `device_tokens`, `conversations`, `chat_messages`. Plan: an anonymous device-token cookie persists FPL team ID + chat history without login; a minimal Google sign-in (`authlib` + signed session cookie) merges a device's history into an account and later gates encrypted BYOK API-key storage (Phase 3). A `conversation` is owned by exactly one of `user_id` or `device_token` (`conversations_owner_check`), so either can be set once a device merges into an account. These 4 tables are deliberately **not** granted to `gaffer_readonly` or `gaffer_etl` — see the `gaffer_app` role gotcha below. `DATABASE_APP_URL` and the actual cookie/OAuth wiring are follow-up PRs — this migration only creates the schema and role.
